...
class ConnectedApp extends Component {
    ...
    componentDidMount() {
        ...
        socket.on("room connection", data => {
            ...
            fetch(new URL('http://localhost:3000/api/cards/white/7'), {
                method: 'POST',
        ...
        socket.on("room timer", data => {
            if (data.timer == 0) {
                if (this.props.roomInfo.room.status == "players selecting cards" && !this.state.didPlay) {
                    ...
                    console.log("here")
                    fetch(new URL(`http://localhost:3000/api/cards/white/${7-this.props.userDeck.deck.length}`), {
                        method: 'POST',
    ...
    newCardPlayed(cardContent) {
        fetch(new URL(`http://localhost:3000/api/cards/new/white`), {
            method: 'POST',
    ...
    connectToRoom(action, roomId = null) {
        fetch(new URL(`http://localhost:3000/room/${action == 'create' ? "new" : roomId}`), {
            method: 'POST',
            body: JSON.stringify({
                socket: socket.id,
